...
router.post('/upload',upload.single('upload'),function(req,res,next){
	...
	if(req.file){
		User.findOneAndUpdate({_id:req.user._id},
			{$set:{image:req.file.filename}},(err,user) => {
			if(err) res.send({msg:"Something went wrong",success:false});
			if(user) {
...
router.post('/update',middleware.update_Valid,function(req,res,nex){
	...
	User.findOneAndUpdate({_id:req.user._id},{$set:{fullname:req.body.fullname,
		username:req.body.username,bio:req.body.bio}},(err,user) => {
		if(err) res.send({msg:"Something went wrong",success:false});
		if(user){
