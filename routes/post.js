...
router.get('/post/:id',function(req,res,next){
  	db.findData(Post,{_id:req.params.id})
  	  .then(function(data){
  	  		res.render('post',{title:"good_deeds",
  	  			user:req.user,header:true,navbar:true,posts:[data],single:true});
  	  })
  	  .catch(function(err){
  	  	  next(err);
  	  });
...
router.post('/savepost',upload.single('upload'),function(req,res,next){
	...
	if(!file){
		return res.send({msg:"Please upload image with post category." ,success:false});
	}
	...
	async.waterfall([
		function(callback){
			var newPost = new Post();
				...
				newPost.category = category.trim();
				newPost.image = file.filename;
				newPost.owner = userdata;
				newPost.save((err,data) => {
					if(err)  res.send({msg:"Something went wrong" ,success:false});
...
router.post('/saveComment',function(req,res,next){
	...
	db.findData(Post,{_id:req.body.id})
	  .then(function(data){
	  	 data.comments.push({user:req.user._id,
	  	 	fullname:req.user.fullname,username:req.user.username,comment:req.body.text});
